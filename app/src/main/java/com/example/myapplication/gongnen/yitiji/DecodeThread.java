package com.example.myapplication.gongnen.yitiji;
...
import com.example.myapplication.gongnen.Constant;
import com.example.myapplication.gongnen.WhitelistBean;
import com.example.myapplication.gongnen.fileSeva.FileSave;
import com.example.myapplication.gongnen.taigan.taigan;
import com.example.myapplication.gongnen.util.HttpURLConnectionUtil;
import com.example.myapplication.gongnen.util.TimeUtil;
...
public class DecodeThread {
	...
	private void yitijiSaveResult(String cameraIp, byte[] carPlate, byte[] img, int height, int width, int left, int top, int resType, int carPlateColor, int nType) {
		Log.e(TAG+99,"出口监听yitijiSaveResult----》》》saveResult");

	    AllcarPlate = readImageData(carPlate);

		//发送请求到zld，如果在白名单里面就开闸
		try{

			String fh =	HttpURLConnectionUtil.doPOST("http://127.0.0.1:8081/zld/whitelist.do?action=seek","cph="+AllcarPlate);

			WhitelistBean resultBean = new Gson().fromJson(fh, WhitelistBean.class);

			if(resultBean.getCode()!="1" && resultBean.getCph()!=null){
				return;
			}
			long s = TimeUtil.getDatePoorMin(resultBean.getEffective_time(),new Date());
	   ...
	   Bitmap srcbmp = BitmapFactory.decodeStream(is);
		...
		if (srcbmp != null){
			...
			Log.e(TAG+124,bd.toString());

			//Log.e(TAG+124,new String(Base64.encodeBase64(img)).length()+new String(Base64.encodeBase64(img)));
	...
	public void yitijiThread(final String cameraIp) {
		...
		new Thread(new Runnable() {
			...
			public void run() {
				...
				String ret = DecodeManager.getinstance().runYitiji(DecodeThread.this, cameraIp);
				if (ret!=null && ret.equals("recvfailed")){
					msg1.arg1 = -1;
				}else if (ret!=null && ret.equals("connecterro")){
					msg1.arg2 = 0;
				}
				handler.sendMessage(msg1);
			}
